...
import java.util.*;
public class BalanceStatement extends JFrame implements ActionListener
{
JTable tb1,tb2;
int l;
JPanel p1,p2;
JTextArea ta1;
String ds1,ds2,type,bst;
int x=400,y=180,w=80,h=20,a,b,st1,en1,st2,en2;
ImageIcon ic;
JLabel l1,l2,l3,l4,l5,l6,l7,profile,ll1,ll2,ll12,ll3,pro,acc,title1;
JButton b1,b2;
JScrollPane sr1,sr2;
String title[]={"Account No","Holder Name","Date","Time","Transfer"};
JComboBox cb1,cb2,cb3,cb4,cb5,cb6,cb7;
String s[]={"All","Deposit","Withdraw"};
...
Statement st;
ResultSet rs;
public BalanceStatement(String bst)
{
try
{
Toolkit tk=Toolkit.getDefaultToolkit();
Dimension dim=tk.getScreenSize();
setSize(dim.width+5,dim.height+5);
this.bst=bst;
...
l2=new JLabel("To:");
l3=new JLabel("Type");
ic=new ImageIcon(new ImageIcon("profile (1).png").getImage().getScaledInstance(50,50,Image.SCALE_DEFAULT));
setIconImage(new ImageIcon("post-office.png").getImage());
profile=new JLabel(ic);
add(profile);
rs=st.executeQuery("select * from info where acc_no='"+bst+"';");
while(rs.next())
{
l5=new JLabel("Account No : "+rs.getString(1).substring(0,3)+"XXXX");
l6=new JLabel("Holder Name : "+rs.getString(2));
l4=new JLabel("Balance Amount : "+rs.getString(3));
}
add(cb1);
add(cb2);
add(cb3);
...
title1.setBounds((p2.getWidth()/2)-200,(p2.getHeight()/2)-40,p2.getWidth(),p2.getHeight());
...
profile.setBounds((getWidth()/2)-150,90,50,50);
l5.setBounds((getWidth()/2)-100,85,200,h);
l6.setBounds((getWidth()/2)-100,103,200,h);
l4.setBounds((getWidth()/2)-100,120,200,h);
...
Date d1=new Date();
rs=st.executeQuery("select * from b_s where acc_no='"+bst+"' and date >= #1990-01-01# and date <= #"+(1900+d1.getYear())+"-"+(1+d1.getMonth())+"-"+(d1.getDate())+"#;");
...
while(rs.next())
{
data[x][0]=rs.getString(1).substring(0,3)+"XXXX";
...
//System.out.println(data[x][0]=rs.getString(1));
x++;
}
if(l==0)
{
...
tb2.setRowSelectionAllowed(false);
if(l!=0)
{
tb2.setRowHeight(20);
}
...
{
tb2.setRowHeight(350);
}
tb2.getTableHeader().setBorder(null);
tb2.getTableHeader().setBackground(Color.black);
tb2.getTableHeader().setForeground(Color.white);
DefaultTableCellRenderer dtcr=new DefaultTableCellRenderer();
dtcr.setHorizontalAlignment(JLabel.CENTER);
for(int i=0;i<title.length;i++)
{
tb2.getColumnModel().getColumn(i).setCellRenderer(dtcr);
}
sr2=new JScrollPane(tb2);
add(sr2);
sr2.setBorder(null);
sr2.setBounds(125,300,1100,400);
sr2.getViewport().setBackground(Color.white);
//&&&&&&&&&&&&&&&
setVisible(true);
}
catch(Exception e)
...
if(type.equals("All"))
{
rs=st.executeQuery("select * from b_s where acc_no='"+bst+"' and date >= cdate('"+ds1+"') and date <= cdate('"+ds2+"');");
}
if(type.equals("Deposit"))
{
rs=st.executeQuery("select * from b_s where  acc_no='"+bst+"' and amount>0 and date >= cdate('"+ds1+"') and date <= cdate('"+ds2+"');");
}
...
add(sr1);
sr1.setBorder(null);
sr1.setBounds(125,300,1100,370);
